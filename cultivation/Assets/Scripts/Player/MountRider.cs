...
/// <summary>
/// **坐骑系统**：按 <see cref="坐骑键"/>（和御风共用的 Shift）上/下坐骑。
///
...
///   "坐骑消散完"同时发生。
///
/// ## 和「凭虚御风」的关系（用户 2026-09-24 改成"互斥 + 一个键"）
///
/// > 「统一改成 shift 开启御风或是坐骑，装备坐骑时会自动停用御风这个被动神通。
/// >   启用凭虚御风时会自动取消装备坐骑」
///
/// 所以：
/// * **Shift 是两者共用的一个键**：装了坐骑 → Shift 归坐骑；没装 → Shift 归御风
///   （<see cref="按了坐骑键"/> 里"没装坐骑就直接 return"，御风那边也用
///   `按键归坐骑()` 对称地让开）；
/// * **装备坐骑 → 自动停用【凭虚御风】**，**启用御风 → 自动取消装备坐骑** ——
///   两条规则都写在 `UIPanelData`（装备 / 启停的唯一入口），所以两者不会同时生效；
///   面板里卸掉坐骑时，这里每帧的兜底判断会**立刻收掉正在骑的那只**；
/// * 骑乘期间仍然**屏蔽 Shift 起飞**（<see cref="YufengFlight.禁止切换"/>），高度由本组件自己管，
///   **不消耗灵气**、**不走重力**；
/// * 玩家动画只是**借用**御风的四个片段（升空/Idle/前进/落地），
///   通过 <see cref="PlayerAnimationController"/> 的外部驱动接口播。
///
/// ## 高度：固定的**对地**高度（用户 2026-09-24 定）
///
/// 骑乘高度 = **每帧向下探到的地面** + <see cref="坐骑摆位.骑乘高度"/>。
/// 上坡下坡跟着地形走（不陷进去也不悬空）；**探不到地面（断崖 / 空洞）时
/// 沿用上一次探到的地面高度** —— 也就是原地维持一个固定高度。
///
/// 以前只有 贴地=true 的坐骑这么算，悬浮坐骑（碧水兽 / 翅膀 / 神龙）用的是
/// 「上坐骑那一刻的地面高度」，地形一变对地高度就不对了 ✗ ——
/// 现在统一成对地高度，`坐骑摆位.贴地` 这个开关**已经废弃、代码不再读**。
///
/// ## 坐骑摆位
///
...
public class MountRider : MonoBehaviour
{
    ...
    [Header("按键")]
    [Tooltip("**上/下坐骑的按键**。★ 用户定：和【凭虚御风】**共用 Shift 一个键** ——\n" +
             "装了坐骑时 Shift 是坐骑，没装坐骑时这一下留给御风（见 按了坐骑键）。")]
    public KeyCode 坐骑键 = KeyCode.LeftShift;

    [Tooltip("第二个键（键盘左右 Shift 都认）。填 None 表示不要")]
    public KeyCode 坐骑键2 = KeyCode.RightShift;
    ...
    public class 坐骑摆位
    {
        ...
        [Tooltip("★ 骑乘时角色根抬离地面的高度（米）——**每只坐骑自己的**。\n" +
                 ...
                 "★ 是**对地**高度：每帧探脚下实际地面，上坡下坡跟着走（见 基准地面高度）")]
        public float 骑乘高度 = 5.64f;
        [Tooltip("【已废弃 · 代码不再读取】旧版「贴地跑」开关。\n" +
                 "用户 2026-09-24 定：**所有坐骑统一维持固定的对地高度** ——\n" +
                 "每帧探脚下地面（探不到就沿用上一次），悬浮和跑地坐骑的算法已经没有区别了。\n" +
                 "字段留着只是不想丢场景里的旧数据，将来会删。")]
    ...
    void Awake()
    {
        ...
        记录地面高度 = transform.position.y;    // 初值，骑上去之后每帧都会被脚下的实际地面覆盖
    }
    ...
    void Update()
    {
        ...
        // ★ Shift 是「坐骑 / 御风」**共用的一个键**：装了坐骑才响应，没装就留给御风
        ...
        //   取消装备之后总不能还让那只坐骑驮着人跑。
        //   面板打开时 timeScale=0，Update 照跑，所以是"当场"收掉（没有过渡动画）。
        if (骑乘中 && 面板数据 != null && 面板数据.当前坐骑 != 坐骑定义)
        {
            if (打印日志) Debug.Log("[坐骑] 面板里换了 / 卸了坐骑 → 立刻收掉正在骑的这只", this);
            强制下坐骑();
        }
    ...
    void 按了坐骑键()
    {
        if (状态 == 骑乘状态.未骑乘)
        {
            // ★ 没装坐骑时**这一下不归我** —— Shift 要留给【凭虚御风】（用户定的共用键）。
            //   以前这里是 E 键，没装坐骑会提示"还没有装备坐骑"；
            //   现在再提示就变成噪音了：玩家按 Shift 想飞，却被坐骑系统抢答一句"没装坐骑"。
            if (面板数据 == null || 面板数据.当前坐骑 == null) return;
            尝试上坐骑();
        }
    ...
    void 尝试上坐骑()
    {
        ...
        // ★ 御风流程中要上坐骑 → **先把御风收干净，不再"开启不了对方"**（用户 2026-09-24 定）。
        //   正常路径上走不到这里：装备坐骑时【凭虚御风】已经被自动停用了
        //   （UIPanelData.设置当前坐骑 → 停用被动 → PlayerAbilityLoader 顺手 强制落地()）。
        //   这条只是数据对不上时的兜底，所以强制落地 + 打一条 warning。
        if (御风 != null && 御风.御风流程中)
        {
            御风.强制落地();
            if (打印日志) Debug.LogWarning("[坐骑] 御风流程中召唤坐骑 —— 已把御风强制落地，再上坐骑", this);
        }
        ...
        记录地面高度 = 起始高度;        // 先给个初值，随后每帧都会被脚下的实际地面覆盖
        已探到地面 = false;
        角色目标高度 = 起始高度 + 骑乘高度;
    ...
    /// <summary>
    /// 高度计算的**基准地面**：**每帧向下探脚下实际的地面**（上坡下坡跟着走，不陷不悬）。
    /// 探不到（断崖 / 空洞 / 悬空）→ **沿用上一次探到的高度**，
    /// 也就是用户要的「检测不到地面时维持一个固定高度」。
    ///
    /// ★ 用户 2026-09-24 定：**所有坐骑统一按这个算** ——
    /// 以前 悬浮坐骑 用的是「上坐骑那一刻的地面高度」（`坐骑摆位.贴地` 那个开关），
    /// 地形一变对地高度就不对了 ✗
    /// </summary>
    float 基准地面高度
    {
        ...
        {
            float y;
            if (GroundProbe.向下取地面(transform.position, 地面探测深度, transform, out y))
            {
                记录地面高度 = y;
                已探到地面 = true;
            }
    ...
    /// <summary>上一次探到的地面高度（探不到时沿用它 → 固定高度）</summary>
    float 记录地面高度;

    /// <summary>本次骑乘有没有真探到过地面（排查 / 自动化验证用：false = 正在靠"固定高度"兜底）</summary>
    ...
    {
        // 高度就锁在这，不走重力、不消耗灵气
        // ★ 每帧都重新取**脚下的实际地面** → 上坡下坡跟着走；
        //   探不到地面（断崖 / 空洞）时 基准地面高度 沿用上一次的值 → 原地维持固定高度
        角色目标高度 = 基准地面高度 + 骑乘高度;
    ...
    {
        过渡计时 = 0f;
        // 下坐骑这一段的下落基准：脚下的实际地面（探不到就用"当前高度 − 骑乘高度"，
        // 也就是这一路维持的那个固定地面）。之后每帧仍由 基准地面高度 继续探。
        记录地面高度 = 角色目标高度 - 骑乘高度;
